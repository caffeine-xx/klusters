...
EventsProvider::EventsProvider(KURL fileUrl,int currentSamplingRate,int position): DataProvider(fileUrl),nbEvents(0),
eventPosition(static_cast<float>(position) / 100.0),modified(false){

 this->currentSamplingRate = static_cast<float>(static_cast<float>(currentSamplingRate) / 1000.0);
...
//If the number of events could not be determined, stop here
 ...
 status = eventFile.open(IO_ReadOnly);
 if(!status){
  events.setSize(0,0);
  timeStamps.setSize(0,0);
  return OPEN_ERROR;
...
void EventsProvider::retrieveData(long startTime,long endTime,QObject* initiator){   
 ...
 while(time <= endTime && startIndex <= nbEvents){
  times(1,count + 1) = static_cast<dataType>(floor(0.5 +(timeStamps(1,startIndex) - static_cast<float>(startTime)) * currentSamplingRate));
...
void EventsProvider::requestNextEventData(long startTime,long timeFrame,QValueList<int> selectedIds,QObject* initiator){  
 ...
 long count = 0;
 while(time <= endTime && startIndex <= nbEvents){
  times(1,count + 1) = static_cast<dataType>(floor(0.5 +(time - startingTime) * currentSamplingRate));
  ids(1,count + 1) = eventIds[events(1,startIndex)];
  count++;
  startIndex++;
  if(startIndex > nbEvents) break;
