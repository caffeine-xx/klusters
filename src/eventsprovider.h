...
public:
  ...
  */
  EventsProvider(KURL fileUrl,int currentSamplingRate,int position = 25);
	~EventsProvider();
  ...
  */  
  inline void updateSamplingRate(int rate){
   currentSamplingRate = static_cast<float>(static_cast<float>(rate) / 1000.0);
